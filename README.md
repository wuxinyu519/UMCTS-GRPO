...
This repository is a UMCTS-GRPO research fork based on Tree-GRPO. It keeps the
original Tree-GRPO training pipeline and adds uncertainty-aware MCTS expansion,
semantic clustering, confidence-calibrated local advantages, and Delta Slurm
launch scripts.

Large runtime artifacts are intentionally not tracked by git. Download datasets,
retrieval indexes, pretrained models, logs, and training outputs under local
directories such as `data/`, `logs/`, and `results/`.

## UMCTS Quick Start

### Environments

Create the training environment following the Tree-GRPO setup:
...
Create a separate retriever environment:
...
Install a FAISS GPU build that is available on your platform. On some clusters,
`faiss-gpu==1.7.3` is not available via pip, so use conda or the closest
compatible build provided by the cluster.

### Data And Models

Use local paths under this repository:

```bash
mkdir -p data/wiki18 data/models logs results run_info
```

Download the Wiki18 retrieval index and corpus as in Tree-GRPO:
...
Place or download local Hugging Face models under `data/models/`, for example:
...
The E5 model is used by both retrieval and UMCTS semantic clustering when
`UMCTS_CLUSTER_MODE=embedding`.

### Running On Delta Slurm

Start the retriever service first:

```bash
sbatch submit_retriever_2gpu.sbatch
```

Then submit UMCTS training. The training job uses 8 GPUs; the retriever uses
separate GPUs and is not included in that count.

Example 4 nodes x 2 GPUs:
...
## UMCTS Implementation Notes

See `README_UMCTS.md` and `UMCTS_SEMANTIC_CLUSTERING.md` for implementation
details. The main code paths are:
...
The original Tree-GRPO README continues below for upstream context.
