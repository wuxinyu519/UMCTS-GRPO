...
DATASET_NAME=${DATASET_NAME:-webagent}
...
train_batch_size=${train_batch_size:-128} # keep original webagent default
val_batch_size=$((total_gpus * 16)) # * 16=256 for 16 GPUs, 128 for 8 GPUs
...
export BASE_MODEL=${BASE_MODEL:-'./Qwen2.5-14B'}
export EXPERIMENT_NAME=${EXPERIMENT_NAME:-webagent-umcts-qwen2.5-14b}
...
ray job submit --address=$RAY_DASHBOARD_ADDRESS \
    ...
    data.train_files=$DATA_DIR/deep_search_2k_filt_sr1.parquet \
    data.val_files=$DATA_DIR/deep_search_test_split_sr1.parquet \
    ...
    trainer.total_training_steps=34 \
    trainer.default_hdfs_dir=null \
